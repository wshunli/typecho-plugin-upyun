 ...
 * 又拍云（UpYun）附件管理插件
 *
 * @package UpyunForTypecho
 * @author wshunli
 * @version 1.0.0
 * @link https://www.wshunli.com
 * @date 2019-01-10
 */
class UpyunForTypecho_Plugin implements Typecho_Plugin_Interface
{
    ...
    {
        Typecho_Plugin::factory('Widget_Upload')->uploadHandle = array('UpyunForTypecho_Plugin', 'uploadHandle');
        Typecho_Plugin::factory('Widget_Upload')->modifyHandle = array('UpyunForTypecho_Plugin', 'modifyHandle');
        Typecho_Plugin::factory('Widget_Upload')->deleteHandle = array('UpyunForTypecho_Plugin', 'deleteHandle');
        Typecho_Plugin::factory('Widget_Upload')->attachmentHandle = array('UpyunForTypecho_Plugin', 'attachmentHandle');
        Typecho_Plugin::factory('Widget_Upload')->attachmentDataHandle = array('UpyunForTypecho_Plugin', 'attachmentDataHandle');
    ...
    {
        $upyundomain = new Typecho_Widget_Helper_Form_Element_Text('upyundomain', NULL, 'https://',
            _t('绑定域名：'),
            _t('绑定 Upyun 服务的域名，注意以 http:// 开头，最后不要加/'));
        $form->addInput($upyundomain->addRule('required', _t('您必须填写绑定域名'))
        ...
        $form->addInput($upyunpwd->addRule('required', _t('您必须填写密码，它是由Upyun提供'))
            ->addRule(array('UpyunForTypecho_Plugin', 'validate'), _t('验证不通过，请核对Upyun操作员和密码是否输入正确')));
    }
    ...
    public static function uploadHandle($file)
    {
        ...
        $settings = $options->plugin('UpyunForTypecho');
    ...
    {
        $domain = Helper::options()->plugin('UpyunForTypecho')->upyundomain;
    ...
    public static function validate()
    {
        ...
        $hostUsage = 0;
        try {
            require_once('upyun-sdk/vendor/autoload.php');
            $upyun = new Upyun(new Config($host, $user, $pwd));
    ...
    {
        $options = Typecho_Widget::widget('Widget_Options')->plugin('UpyunForTypecho');
        require_once('upyun-sdk/vendor/autoload.php');
        return new Upyun(new Config($options->upyunhost, $options->upyunuser, $options->upyunpwd));
    }
